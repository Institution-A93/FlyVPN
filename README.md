...
- DNS клиентов резолвит сам sing-box через публичный резолвер (резолв на egress; unbound и nginx убраны — ADR-0011/0017)

Логика дата-плейна (proxy-режим, без kernel-NAT/TUN — ADR-0015):
1. sing-box принимает VLESS-соединения из mesh-туннеля
2. DNS-запросы клиентов sing-box резолвит сам (outbound `direct` на публичный резолвер)
3. Прочий трафик — outbound `direct`: соединение инициирует egress, выход с его публичного IP
4. Приватные сети заблокированы routing-правилом (`ip_is_private` → block)
