...
> переводит её на FreeRADIUS-native (PostgreSQL) + тонкий Go-сервис (панель +
> идемпотентный контракт). Разделы ниже сохранены как история и контракт периферии,
> но в части entitlement читать через призму ADR-0021.**
...
- **Движок** — каноническая FreeRADIUS-схема на PostgreSQL 16
  (`radcheck`/`radacct`/`radusergroup`); EAP-MSCHAPv2 из `NT-Password` (ADR-0014);
  квота — `rlm_sqlcounter`; отрезание — CoA (strongSwan `dae`). Подписка =
  `Expiration` + кап, без переноса; продление = сдвинуть срок + сбросить счётчик.
- **Операторская поверхность** — **тонкий Go-сервис** (ADR-0013): панель для
  нетехнического суппорта (найти юзера, статус, трафик, сессии, продлить/выключить) +
  **идемпотентный контракт** `renew/provision/revoke` над `radcheck` (единственный
  писатель; кнопки панели зовут его же).
...
| Оркестратор: cron расхода/сброса/порогов/реф-расчёта | нативный счётчик + CoA; у оркестратора остаётся узлы/health/ротация/провизия |
| `account-api`: auth/сессии/entitlements/devices | снять; доступ — нативный FreeRADIUS |
| Миграции `0003`–`0005` | superseded |
| `account-api` целиком | тонкий Go-сервис (панель+контракт) + коннекторы; `.mobileconfig`/`.sswan` — в `config-api` |
