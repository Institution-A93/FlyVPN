...
## ⚠️ Пересмотр скоупа (ADR-0021/0022/0023) — заменяет план ниже

> **Этот документ (draft v3) описывает самописную Go-плоскость entitlement. ADR-0021
> переводит её на FreeRADIUS-native (PostgreSQL) + ОДИН Go-бинарь `control` (панель
> CRUD + контракт + профили + флот). Разделы ниже сохранены как история и контракт
> периферии, но в части entitlement читать через призму ADR-0021.**

**Задача тривиальна:** подписка = лимит времени + лимит МБ, без переноса; оплата →
продление. Коммерческий учёт **вне MVP** (самописные коннекторы). В скоупе —
технический учёт. Плоскость доступа — нативная (FreeRADIUS + PostgreSQL,
спайк-проверено), без платформ поверх (OpenWISP/SHM/BSS/daloRADIUS — оверхед или
половина задачи, см. `docs/research/billing-and-bss-options.md` и
`docs/research/radius-admin-panels.md`).

**Финальная форма:**

- **Движок** — нативный FreeRADIUS на PostgreSQL 16, **своя `auth_credentials`
  (остаётся, прошита в `sql.j2`)** + `radacct` + `Expiration`; EAP-MSCHAPv2 из
  `NT-Password` (ADR-0014); квота — `rlm_sqlcounter`; отрезание — CoA (strongSwan
  `dae`). Подписка = `Expiration` + кап, без переноса; продление = сдвинуть срок +
  сбросить счётчик. (Канонический `radcheck` не нужен — своя панель.)
- **Операторская поверхность** — **ОДИН Go-бинарь `control`** (модульный монолит,
  ADR-0013): `panel` (**ручной RADIUS-aware CRUD юзеров** через формы + обзор узлов,
  операторская авторизация) + `contract` (**CRUD** create/read/update/delete над
  `auth_credentials` — единственный писатель; панель и коннекторы зовут его же) +
  `profiles` (`.mobileconfig`/`.sswan`) + `fleet` (health/реестр/автопровизия).
  `config-api` и `orchestrator` сливаются в него пакетами.
- **Коммерческий учёт** — вне MVP → самописные коннекторы (платёжные адаптеры) зовут
  `renew()`; добавить способ оплаты = правка только коннекторов, не живой инфры.
- **Клиентская доставка** (ADR-0022) — iOS `.mobileconfig` / Android strongSwan
  `.sswan`, оба генерит `config-api`, доставляет Telegram-бот.
- **Узлы** (ADR-0023) — симметричные: одна IaC-роль `node` (регион + вход/выход),
  двунаправленный РФ↔загран; оркестратор выбирает exit по направлению.
...
|-----------------|------------------------|
| `usage_log`, bespoke `subscriptions`/quota | своя `auth_credentials` + `radacct` + `rlm_sqlcounter` |
| Оркестратор: cron расхода/сброса/порогов/реф-расчёта | нативный счётчик + CoA; реестр/health/автопровизия → пакет `fleet` в `control` |
| `account-api`: auth/сессии/entitlements/devices | снять; донор каркаса (HTTP/store/NTHash/JWT) для `control` |
| `config-api`, `orchestrator` как отдельные сервисы | сливаются в `control` пакетами (`profiles`/`contract`, `fleet`) |
...
**ADR-0005/0014/0013** (переутверждены), **ADR-0020** (Platega — один из коннекторов
за `renew()`, а не своя модель подписок).
