...
//
// Зеркалит services/config-api/internal/credentials: NT-hash должен совпадать байт-в-байт,
// иначе FreeRADIUS не примет кред. Дублируется намеренно — cross-module import internal/
// невозможен, а config-api по §9 backend-requirements остаётся нетронутым.
package credentials
