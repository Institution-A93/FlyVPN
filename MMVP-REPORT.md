...
| Сквозной путь (iPhone WiFi+LTE) | IKEv2/EAP-RADIUS → сплит: заграница через egress, РФ напрямую, DNS, MTU — проверено вживую 2026-06-18 | ✅ |
| OpenTofu — Hetzner (egress/control) + Selectel (ingress) | развёрнуто реальным `apply` через CI | ✅ |

---

## 4. Не входило в MMVP / осталось

- **Госсайты/банки (Госуслуги)**: блокируют IP дата-центров (в т.ч. Selectel) — недоступны
  даже напрямую с ingress. Это **не баг маршрутизации** (сплит корректен), а отказ на стороне
  назначения; чинится только резидентным выходом. Подробно — backlog **B4**.
- **Маршрутизация по GeoIP-CIDR, не ASN**: точность ограничена GeoIP-списками — backlog **B3**.
